...
class ProposedScene(BaseModel):
    ...
    location: str = Field(default="", description="שם המקום הפיזי שבו מתרחשת הסצנה (למשל: מחסן, חדר, רחוב, שדה)")
...
SYSTEM_PROMPT = (
    ...
    "עקרונות הבמאי לבחירת תוכן התמונה (כך תבחר מה כל תמונה תציג — בלי לשנות את כמות הסצנות):\n"
    "- **עגן כל סצנה במקום פיזי**: זהה את המקום שבו מתרחש הקטע (עיר, רחוב, מחסן, בית, חדר, שדה — הסיפור כמעט תמיד אומר זאת במפורש) ובנה את התמונה סביבו. מלא את שם המקום בשדה location.\n"
    "- **קישור מקום לרפרנס (עקביות מקומות)**: כדי שאותו מקום ייראה זהה בכל פעם שהוא חוזר, עליך לקשר לכל סצנה רפרנס-מקום (קטגוריה style) ב-references בפורמט 'ID|שם', בדיוק כמו שאתה עושה לדמויות. אם המקום כבר קיים באינדקס — השתמש ב-ID שלו; אם זהו מקום מרכזי/חוזר שעדיין לא באינדקס — הצע אותו ב-new_references (קטגוריה style). **אל תיצור רפרנס-מקום למקום שולי שמופיע פעם אחת בלבד** כדי לא להציף את האינדקס; במקרה כזה מלא רק את שדה location בטקסט. ודא ש-location והרפרנס-מקום המקושר עקביים ביניהם (אותו מקום = אותו ID).\n"
    "- **דמויות עיקריות**: כלול בתמונה את הדמויות שנמצאות במקום רוב הזמן; התעלם מדמויות שנכנסו ויצאו לרגע.\n"
    "- **בחר את הרגע הדרמטי**: מתוך כל מה שקורה בקטע, זהה את הפעולה המרכזית (מחפשים/מדברים/רבים/משחקים) והעדף להציג את הפעולה הדרמטית / הנדירה / המיוחדת / המצחיקה ביותר (קרב על פני שיחה, בריחה על פני משחק וכו').\n"
    "- **הימנע מספוילר**: אם הקטע מכיל סוד או דבר מוסתר/סמוי, אל תצייר את הפעולה שתחשוף את סוף הקטע — בחר רגע ניטרלי שאינו מגלה את ההפתעה.\n\n"
